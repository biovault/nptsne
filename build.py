...
def _is_not_VS15MDonWindows(build):
    return not (
        build.settings["compiler.version"] == "15"
...
if __name__ == "__main__":
    ...
    if not branch.startswith("release"):
        new_reference = "{}/{}@{}/{}".format(
            name, version, username, branch.replace("/", "_")
        )
        print("Generated reference: ", new_reference)

    builder = build_template_default.get_builder(
        docker_entry_script=docker_entry_script, reference=new_reference
    )
    ...
    builder.update_build_if(
        lambda build: True, new_env_vars={"CONAN_SOURCE_BRANCH": branch}
    )
    builder.run()
