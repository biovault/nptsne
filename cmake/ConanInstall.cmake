...
if(NOT CONAN_DETECT_QUIET)
    message(STATUS "Conan Install: Found program ${CONAN_CMD}")
endif()

set(CONAN_PATH $ENV{CONANPATH})
message(STATUS "Got conan path ${CONAN_PATH}")
execute_process(COMMAND ${CONAN_PATH} "--version" OUTPUT_VARIABLE CONAN_VERSION_OUTPUT RESULT_VARIABLE CONAN_RESULT)
message("Tested version -${CONAN_VERSION_OUTPUT}- -${CONAN_RESULT}-")

message("Test version")
file(TO_NATIVE_PATH "${CONAN_CMD}" CONAN_CMD)
string(REPLACE "\\" "\\\\" CONAN_CMD "${CONAN_CMD}" )
message("Will call ${CONAN_CMD}")
execute_process(COMMAND ${CONAN_CMD} "--version" OUTPUT_VARIABLE CONAN_VERSION_OUTPUT RESULT_VARIABLE CONAN_RESULT)
message("Tested version -${CONAN_VERSION_OUTPUT}- -${CONAN_RESULT}-")

find_package(Python3 COMPONENTS Interpreter REQUIRED)
#Quote the conan path invocation (to avoid windows backslash issues)
execute_process(COMMAND ${PYTHON3_EXECUTABLE} -c "import fileinput; for line in fileinput.input(['conan.cmake'], inplace=True): print(line.replace('COMMAND ${CONAN_CMD}', line.replace('COMMAND \"${CONAN_CMD}\")")
