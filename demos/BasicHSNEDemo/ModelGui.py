...
class ModelGui:
    ...
    def run(self):
        ...
        self.gobtn.grid(
            column=0, row=3, columnspan=1, rowspan=1, sticky="sew", in_=self.control_frame
        )
    ...
    def load(self, event):
        ...
        self.name = filedialog.askopenfilename(
            ...
            title="Open a numpy file where each row is a data point and columns are dimensions",
        )
    ...
    def load_labels(self, event):
        ...
        self.label_name = filedialog.askopenfilename(
            initialdir=workdir,
            filetypes=[("Numpy files", "*.npy")],
            title="Open a numpy file where each row is an integer label",
        )
